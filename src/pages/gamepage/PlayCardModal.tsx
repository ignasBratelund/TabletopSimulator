...
function incrementTurn(game: GameDTO){
    if (game.players.filter(player => player.hand.length > 0).length >= 2){
        ...
        if(game.drawPile.length >= 1){
            ...
            for (let i = 0; i < game.players.length; i++){
                ...
                if (game.players[i].hand.length > 0){
                    game.log.push({action: game.players[i].name + " has a " + game.players[i].hand, timestamp: new Date()});
                ...
                if (leadingPlayersIndex.length === 1){
                    game.log.push({action: game.players[leadingPlayersIndex[0]].name + " won the round", timestamp: new Date()});
                    game.players[leadingPlayersIndex[0]].score += 1;
                }
                else {
                    game.log.push({action: "The round was a tie between " + leadingPlayersIndex.map(i => game.players[i].name).join(", "), timestamp: new Date()});
        ...
        game.log.push({action: game.players[winningPlayerIndex].name + " won the round", timestamp: new Date()});
...
function resolveCard(game: GameDTO, setCard:  React.Dispatch<React.SetStateAction<CardDTO | undefined>>, card: CardDTO, selectedCard: (1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | -1), selectedOpponent: string, playerName: string | null, setError: React.Dispatch<React.SetStateAction<string>>, setGameLog: React.Dispatch<React.SetStateAction<action[]>>){
    setError("")
        ...
        for (let i = 0; i < game.players.length; i++){
            if(game.players[i].name === selectedOpponent){
                if (game.players[i].hand[0] === selectedCard){
                    game.players[i].hand = [];
                    game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and guessed correctly that " + game.players[i].name + " had a " + cardInfo.get(selectedCard)?.name, timestamp: new Date()});
                } else {
                    game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and guessed incorrectly that " + game.players[i].name + " had a " + cardInfo.get(selectedCard)?.name, timestamp: new Date()});
                }
            }
        }
    ...
    if (card.number === 2){
        for (let i = 0; i < game.players.length; i++){
            if(game.players[i].name === selectedOpponent){
                game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and looked at " + game.players[i].name + "'s hand", timestamp: new Date()});
                setGameLog((log) => log.concat({action: game.players[i].name + "'s hand: " + cardInfo.get(game.players[i].hand[0])?.name, timestamp: new Date()}));
            }
        }
        removeCardFromHand(game, playerName, card.number);
    }
        ...
        for (let i = 0; i < game.players.length; i++){
            if(game.players[i].name === selectedOpponent){
                ...
                if (game.players[i].hand[0] < getPlayer(game, playerName)?.hand[0]!){
                    game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and won the duel against " + game.players[i].name + ". " + game.players[i].name + " had a " + game.players[i].hand[0], timestamp: new Date()});
                    game.players[i].hand = [];

                } else if (game.players[i].hand[0] > getPlayer(game, playerName)?.hand[0]!){
                    game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and lost the duel against " + game.players[i].name + ". " + getPlayer(game, playerName)?.name + " had a " + cardInfo.get(getPlayer(game, playerName)?.hand[0]!)?.name, timestamp: new Date()});
                    game.players[getPlayerIndex(game, playerName)].hand = [];
                } else {
                    game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and tied the duel against " + game.players[i].name, timestamp: new Date()});
                }
            }
        }
    }
    ...
    if (card.number === 4){
        ...
        game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and protected themselves until their next turn", timestamp: new Date()});
        removeCardFromHand(game, playerName, card.number);
    }
    //Prince
    if (card.number === 5){
    ...
    if (card.number === 7){
        for (let i = 0; i < game.players.length; i++){
            if(game.players[i].name === selectedOpponent){
                game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " and swapped hands with " + game.players[i].name, timestamp: new Date()});
                removeCardFromHand(game, playerName, card.number);
    ...
    //Countess
    if ([6, 8, 9].includes(card.number)){
        game.log.push({action: playerName + " played the " + cardInfo.get(card.number)?.name + " to no effect", timestamp: new Date()});
        removeCardFromHand(game, playerName, card.number);
    }
    incrementTurn(game);
...
export function PlayCardModal( {card, setCard, changeCard, game, setGameLog} : {card: CardDTO | undefined, setCard:  React.Dispatch<React.SetStateAction<CardDTO | undefined>> ,changeCard: (card: (1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | -1)) => void, game: GameDTO, setGameLog: React.Dispatch<React.SetStateAction<action[]>>}) {
    ...
    return (
        <Dialog sx={{top : "-50%"}} open={Object.keys(card).length > 0} onClose={handleClose}>
            ...
            <DialogActions>
                <Button type="submit" onClick={() => resolveCard(game, setCard, card, selectedCard, selectedOpponent, playerName, setError, setGameLog)}>Play</Button>
            </DialogActions>
        </Dialog>
    )
}
    ...
    <FormControl variant="standard" className="width-100">
        ...
        >
            {game.players.map(player => {
                    if ((showSelf || player.name !== playerName) && player.hand.length > 0){
